...
extern "C"
void app_main_prompt(void)
{
    vTaskDelay(pdMS_TO_TICKS(1000));
    UART s;
    ESP32_LineInput input(s);

    while (true)
    {
        s.write_bytes("PROMPT# ", 8);
        std::string data = input.get(true);

        if (data[data.length() - 1] == '?')
        {
            std::string start_string = data.substr(0, data.length() - 1);
            input.set_string(start_string);
        }
